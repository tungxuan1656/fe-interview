...
Không chỉ biết virtualization mà còn hiểu measurement, overscan và UX constraints.
...
- Listener tự unsubscribe trong lúc emit?
...
- Date/Map/Set?
- NaN và `-0`?
- Có nên dùng JSON.stringify không?
...
- Retry mọi HTTP status có đúng không?
- Jitter để làm gì?
- POST có retry an toàn không?
- Timeout kết hợp retry thế nào?

### Interviewer looks for

Distributed-systems thinking: idempotency, retry storms, timeout và backoff.
...
- Batched updates?
- Persistence?
- Server state có nên đưa vào store không?
...
- `requestIdleCallback` có đủ không?
- Yielding giữa tasks?
- Khi nào `startTransition` phù hợp hơn?
...
- Streaming?
- Memoization?
- Có cần transform toàn bộ data không?

### Interviewer looks for

Biết tối ưu pipeline thay vì chỉ tối ưu một function.
...
**Senior không nhất thiết phải code nhanh nhất.** Một ứng viên mạnh thường làm rõ phạm vi, chọn solution đơn giản trước và biết nói phần nào chưa implement để tránh giả vờ rằng bài toán đã được giải hoàn toàn.
